...
// -----------------------------
// Personal-only AirNav Radar HTML fallback
// -----------------------------
// AirNav's public flight/registration pages expose operational ATC callsigns
// such as QLK27D together with route information. This is useful when ADSBDB
// and FlightAware only know the commercial/public flight number.
const AIRNAV_SCRAPE_ENABLED =
  String(process.env.AIRNAV_SCRAPE_ENABLED || 'true').toLowerCase() !== 'false';
const AIRNAV_SCRAPE_CACHE_MS = 6 * 60 * 60 * 1000;
const AIRNAV_SCRAPE_NEGATIVE_CACHE_MS = 5 * 60 * 1000;
const airNavScrapeCache = new Map();
const airNavScrapeNegativeCache = new Map();
const AIRNAV_HTTPS_AGENT = new https.Agent({ family: 4, keepAlive: true });

function stripPageHtml(value) {
  return decodeHtml(
    String(value || '')
...
function airNavAirportMatches(text) {
  const matches = [];
  const re = /([A-Z][A-Za-z0-9 .,'’\/-]{1,70}?)\s*\(([A-Z0-9]{3,4})(?:\s*\/\s*([A-Z0-9]{3,4}))?\)/g;
  let match;

  while ((match = re.exec(text)) !== null) {
    const codes = [match[2], match[3]].filter(Boolean).map(v => v.toUpperCase());
    const iata = codes.find(code => /^[A-Z]{3}$/.test(code)) || null;
    const icao = codes.find(code => /^[A-Z0-9]{4}$/.test(code)) || null;
    let name = String(match[1] || '').trim();

    name = name
      .replace(/^.*?(?:AGO|LIVE|LANDED|DEPARTED|ARRIVED|SCHEDULED|ESTIMATED|Flight info)\s+/i, '')
      .replace(/^\d{1,2}:\d{2}\s+/i, '')
      .trim();

    if (!iata && !icao) continue;
    matches.push({ iata, icao, name: name || null, municipality: null, country: null });
  }

  return matches;
}

function parseAirNavRoute(html, callsign, registration) {
  const text = stripPageHtml(html);
  if (!text) return null;

  const upper = text.toUpperCase();
  const cleanCallsign = normaliseCallsign(callsign);
  const cleanRegistration = String(registration || '').replace(/\s+/g, '').toUpperCase();

  let anchor = cleanCallsign ? upper.indexOf(cleanCallsign) : -1;

  // Do not guess from an aircraft page unless the exact operational callsign
  // is actually present. Registrations can fly many different routes in a day.
  if (cleanCallsign && anchor < 0) return null;
  if (anchor < 0 && cleanRegistration) anchor = upper.indexOf(cleanRegistration);
  if (anchor < 0) return null;

  const windowStart = anchor;
  const window = text.slice(windowStart, anchor + 3500);
  const airports = airNavAirportMatches(window);

  if (airports.length >= 2) {
    const origin = { ...airports[0] };
    const destination = { ...airports[1] };

    // Some current-flight pages show the IATA pair separately (e.g. "BNE BDB")
    // while the named locations only contain ICAO codes (YBBN/YBUD).
    if (!origin.iata || !destination.iata) {
      const anchorInWindow = Math.max(0, anchor - windowStart);
      const afterAnchor = window.slice(anchorInWindow, anchorInWindow + 900);
      const pair = afterAnchor.match(/(?:Flight info\s+)?\b([A-Z]{3})\s+([A-Z]{3})\b/);
      if (pair) {
        if (!origin.iata) origin.iata = pair[1];
        if (!destination.iata) destination.iata = pair[2];

        // Remove status/header debris that sometimes gets flattened into the
        // first airport name before the IATA pair.
        const pairPrefix = new RegExp(`^.*\\b${pair[1]}\\s+${pair[2]}\\s+`, 'i');
        origin.name = String(origin.name || '').replace(pairPrefix, '').trim() || origin.name;
      }
    }

    return { origin, destination, source: 'airnav-scrape' };
  }

  return null;
}

async function fetchAirNavHtml(url, label) {
  const controller = new AbortController();
  const timeoutHandle = setTimeout(() => controller.abort(), 8000);
  ...
  try {
    const res = await fetch(url, {
      ...
      agent: AIRNAV_HTTPS_AGENT,
      redirect: 'follow',
      signal: controller.signal
    });

    if (!res.ok) {
      console.warn(`[AirNavFallback] ${label} returned HTTP ${res.status}`);
      return null;
    }

    const html = await res.text();
    if (/your session has timed out|captcha|access denied/i.test(html || '')) {
      console.warn(`[AirNavFallback] ${label} returned a session/block page instead of flight data`);
      return null;
    }
    if (!html || html.length < 500) {
      console.warn(`[AirNavFallback] ${label} returned an unexpectedly short page (${html?.length || 0} bytes)`);
      return null;
    }

    return html;
  } catch (err) {
    console.warn(`[AirNavFallback] ${label} failed: ${err?.message || err}`);
    return null;
  } finally {
    clearTimeout(timeoutHandle);
  }
...
async function lookupAirNavScrape(callsign, registration) {
  if (!AIRNAV_SCRAPE_ENABLED) return null;

  const cleanCallsign = normaliseCallsign(callsign);
  const cleanRegistration = String(registration || '').replace(/\s+/g, '').toUpperCase();
  if (!cleanCallsign || cleanCallsign.length < 3) return null;

  const cacheKey = `${cleanCallsign}:${cleanRegistration || '-'}`;
  const now = Date.now();
  const cached = airNavScrapeCache.get(cacheKey);
  if (cached && now - cached.timestamp < AIRNAV_SCRAPE_CACHE_MS) return cached.data;

  const failedAt = airNavScrapeNegativeCache.get(cacheKey);
  if (failedAt && now - failedAt < AIRNAV_SCRAPE_NEGATIVE_CACHE_MS) return null;

  let result = null;

  const flightUrl = `https://www.airnavradar.com/flight/${encodeURIComponent(cleanCallsign)}`;
  const flightHtml = await fetchAirNavHtml(flightUrl, `flight ${cleanCallsign}`);
  if (flightHtml) result = parseAirNavRoute(flightHtml, cleanCallsign, cleanRegistration);

  if (!result && cleanRegistration) {
    const regUrl = `https://www.airnavradar.com/data/registration/${encodeURIComponent(cleanRegistration)}`;
    const regHtml = await fetchAirNavHtml(regUrl, `registration ${cleanRegistration}`);
    if (regHtml) result = parseAirNavRoute(regHtml, cleanCallsign, cleanRegistration);
  }

  if (result?.origin && result?.destination) {
    airNavScrapeCache.set(cacheKey, { timestamp: now, data: result });
    airNavScrapeNegativeCache.delete(cacheKey);
    console.log(
      `[AirNavFallback] Resolved ${cleanCallsign}` +
  ...
  airNavScrapeNegativeCache.set(cacheKey, now);
  console.log(
    `[AirNavFallback] No route found for ${cleanCallsign}` +
    `${cleanRegistration ? ` (${cleanRegistration})` : ''}; caching miss for 5 minutes`
  );
  return null;
}
          ...
          await Promise.all(
            states.map(
              async s => {
                ...
                // AirNav is tried first because it exposes ATC callsigns directly;
                // FlightAware remains a final fallback.
                if (
                  flightNo &&
                  (origin === 'Unknown' || destination === 'Unknown')
                ) {
                  const scrapedRoute =
                    (await lookupAirNavScrape(flightNo, registration)) ||
                    (await lookupFlightAwareScrape(flightNo));
                  if (scrapedRoute) {
                    if (origin === 'Unknown') {
